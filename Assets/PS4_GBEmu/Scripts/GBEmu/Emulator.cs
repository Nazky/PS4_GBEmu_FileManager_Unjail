...
namespace UnityGB
{
	...
	{
		const float FRAMES_PER_SECOND = 59.7f;//59.7f
		const int WIDTH = 160;
		const int HEIGHT = 144;
		int MAX_FRAMES_SKIPPED =10;//10
		...
		private int freq1 = 800;//800
		private int freq2 = 1720;//1720
		private int freq3 = 2040;//2040
		private int freq4 = 4560;//4560
		...
		private void UpdateModel(bool updateBitmap)
		{
			if (updateBitmap)
			{
				...
				for (int y = 0, pixelIndex = 0; y < HEIGHT; ++y)
				{
					...
					ExecuteProcessor(freq1);
					x80.lcdcMode = LcdcModeType.TransferingData;
					ExecuteProcessor(freq2);
